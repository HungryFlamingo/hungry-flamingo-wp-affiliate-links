...
class AdminInit {
	...
	public static function admin_init() {
		if ( ! current_user_can( 'manage_options' ) ) {
			...
			\HungryFlamingo\WpAffiliateLinks\Enqueue::enqueue_admin_css();
			\HungryFlamingo\WpAffiliateLinks\Enqueue::enqueue_admin_js();
	...
	private static function register_menu_pages() {
		 $admin_pages_obj = new \HungryFlamingo\WpAffiliateLinks\AdminPages();
		...
		if ( empty( $GLOBALS['admin_page_hooks']['hungry-flamingo'] ) ) {
			$menu_page = add_menu_page(
				...
				\HungryFlamingo\WpAffiliateLinks\Utils::$plugin_url . 'assets/img/hungry-flamingo-icon.svg',
				null
			);
		...
		$submenu_page[2] = add_submenu_page(
			'hungry-flamingo',
			__( 'Affiliate links - Hungry Flamingo', \HungryFlamingo\WpAffiliateLinks\Utils::$plugin_slug ),
			'Affiliate links',
			'manage_options',
